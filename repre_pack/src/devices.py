...
def model_build_devices():
    ...
    with open('/home/cbas/ROS_Workspaces/nodes_prueba/src/repre_pack/src/pesos_classifier.pkl', 'rb') as f:
...
def model_build_levels():
    ...
    classifier2 = BERTForClassification(model2, num_classes=8)
    classifier2.compile(
    optimizer=tf.keras.optimizers.Adam(learning_rate=1e-5),
    loss=tf.keras.losses.SparseCategoricalCrossentropy(),
    metrics=['accuracy'] )
...
def get_levels(instrucition, tokenizer, classifier2):

    inputs = tokenizer(instrucition, padding=True, truncation=True, return_tensors="tf")
    predicciones = classifier2(inputs)

# Obtener las clases predichas
    niveles_predichas = np.argmax(predicciones, axis=1)


# Imprimir las clases predichas
    print("Nivel predicha:", niveles_predichas)
    return(niveles_predichas)
