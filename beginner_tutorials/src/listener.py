...
def callback(image_msg):
    ...
    cv2.imshow('ROS Image Subscriber 2', cv_image)
    cv2.waitKey(1)  # Wait for 1 millisecond instead of 1000 (no delay)

if __name__ == '__main__':
    rospy.init_node("image_subscriber", anonymous=False)
    print("Subscribe images from topic /image_raw_alejandro_1 ...")

    image_subscriber1 = rospy.Subscriber("image_raw_alejandro", Image, callback)
